...
import { getSupportTickets, getSupportTicket, createSupportTicket, updateSupportTicket } from '../../utils/api';
...
const SupportTickets = () => {
  ...
  const handleUpdateStatus = async (newStatus) => {
    ...
    try {
      ...
      const result = await updateSupportTicket(selectedTicket.id, data);
      setSelectedTicket(result);
    ...
    <div className="w-full space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[#5A5A5A]">Submit and track your support requests</p>
        </div>
        <button
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="relative">
          <select
            ...
            className="w-full sm:w-40 rounded-lg border border-[#D8D4CD] bg-white py-2.5 pl-3 pr-8 text-sm text-[#1A1A1A] outline-none focus:border-[#008751] transition-colors appearance-none"
          >
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-[#E8E3DC]">
              {loading ? (
                ...
                <tr>
                  <td colSpan="5" className="px-4 py-8 text-center text-[#5A5A5A]">No tickets found</td>
                </tr>
              ) : (
                tickets.map((ticket) => (
