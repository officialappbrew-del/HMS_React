  ...
  return <main className="min-h-screen bg-[#F7F5F2] p-4 sm:p-6 lg:p-8"><div className="mx-auto max-w-7xl">
    ...
    {['transfer', 'discharge'].includes(tab) && <section className="grid gap-6 lg:grid-cols-[1fr_380px]">
      ...
      <form onSubmit={tab === 'transfer' ? submitTransfer : submitDischarge} className="border border-slate-200 bg-white p-5">
        ...
        <button disabled={working || !selectedStay} className="mt-4 flex items-center gap-2 bg-[#1C2B27] px-4 py-2 text-xs font-semibold text-white disabled:cursor-not-allowed disabled:opacity-40">
          {working ? <Loader2 size={14} className="animate-spin" /> : <Send size={14} />}
          {working ? (tab === 'transfer' ? 'Transferring patient...' : 'Completing discharge...') : (tab === 'transfer' ? 'Transfer patient' : 'Complete discharge')}
        </button>
      </form>
    </section>}
