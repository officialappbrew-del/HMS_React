...
const Appointments = () => {
  ...
  useEffect(() => {
    if (showStatusMenu === null) return undefined;

    const closeStatusMenu = (event) => {
      if (!event.target.closest('[data-status-control]')) {
        setShowStatusMenu(null);
      }
    };
    const closeOnEscape = (event) => {
      if (event.key === 'Escape') setShowStatusMenu(null);
    };
  ...
  const renderStatusControl = (appointment) => (
    <div data-status-control className="inline-flex min-w-[132px] flex-col items-start gap-1">
      ...
      {showStatusMenu === appointment.id && (
        <div className="z-20 w-full max-w-[calc(100vw-2rem)] rounded-lg border border-gray-200 bg-white py-1 shadow-lg">
          <button
